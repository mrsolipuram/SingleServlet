...
<body>
<img alt="Image" src="./images/sports-cars-wallpapers.jpg" width="200" height="200">
<form action="./signUp" method="post">
<p>First Name<input type="text"name="firstName"/></p>
<p>EmailId:<input type="text" name="emailId"></p>
...
<br>
Jquery File upload
<form action="./upload" method="post" id="file_upload_1"enctype="multipart/form-data">
<div id="drop_zone_1">
<div class="fakeupload">
                                  <input type="text" name="fakeupload1" id="fakeupload1" readOnly="readonly"/>
                                  <span class="browseBtn">Browse</span> 
                                  
                                  <input type="file" name="file_1" id="file_1" size="50" onchange="this.form.fakeupload1.value = this.value;" />
                                  </div>
...
function getDispatchData(){
	$.getJSON('./mixCont',{parameter:'getJson'},function(data){
		alert(data);
		$("#result1").html(data.name);
	});
}
function getStudentData(){
	$.getJSON('./student',{parameter:'getStudentJson'},function(data){
		alert(data);
		$("#result2").html(data.name);
	});
}
function getStudentDynaData(){
	$.getJSON('./studentDyna',{parameter:'studentJSON'},function(data){
		alert(data);
    ...
    $('#file_upload_'+suffix).fileUploadUI({
       ...
       buildDownloadRow: function (file) {  
    	$('#drop_zone_' + suffix).show(); 
     	...
     	var tableObject=document.getElementById('files_'+suffix);
     	$("#uploadImg_"+suffix).attr('src',file.filePath);
     	$("#uploadImg_"+suffix).attr('alt',file.name);
     	if(tableObject.rows.length>0){
       		tableObject.deleteRow(0);	
       	}        
           return $('');
